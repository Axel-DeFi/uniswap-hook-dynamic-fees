...
call_hook_getters() {
  ...
  local floor_fee cash_fee extreme_fee paused current_fee
  ...
  floor_fee="$(cast_call_single "${hook}" "floorFee()(uint24)")"
  cash_fee="$(cast_call_single "${hook}" "cashFee()(uint24)")"
  extreme_fee="$(cast_call_single "${hook}" "extremeFee()(uint24)")"
  ...
  printf 'fee_idx=%s hold_remaining=%s up_extreme_streak=%s down_streak=%s emergency_streak=%s period_start=%s period_vol=%s ema_vol=%s r_bps=%s current_fee_bips=%s paused=%s floor_fee_bips=%s cash_fee_bips=%s extreme_fee_bips=%s period_seconds=%s ema_periods=%s lull_reset_seconds=%s deadband_bps=%s min_closevol_cash=%s up_r_cash=%s cash_hold=%s min_closevol_extreme=%s up_r_extreme=%s up_extreme_confirm=%s extreme_hold=%s down_r_extreme=%s down_extreme_confirm=%s down_r_cash=%s down_cash_confirm=%s emergency_floor=%s emergency_confirm=%s hook_fee_percent=%s\n' \
    "${fee_idx}" "${hold_remaining}" "${up_streak}" "${down_streak}" "${emergency_streak}" \
    "${period_start}" "${period_vol}" "${ema_vol}" "${rbps}" "${current_fee}" "${paused}" \
    "${floor_fee}" "${cash_fee}" "${extreme_fee}" "${period_seconds}" "${ema_periods}" \
