...
contract RunSmokeSwapsLocal is Script {
    function run() external {
        LoggingLib.phase("local.smoke");
        ...
        uint256 swapAmountRaw = vm.envOr("SMOKE_SWAP_STABLE_RAW", range.maxSwapStableRaw);
        if (swapAmountRaw == 0) {
            swapAmountRaw = 1_000_000;
        }
        ...
        require(snapshot.initialized, "hook should stay initialized");

        LoggingLib.ok("smoke swap complete");
    }
